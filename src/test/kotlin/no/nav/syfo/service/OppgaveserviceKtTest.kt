...
class OppgaveserviceKtTest :
    FunSpec({
        ...
        val oppgaveClient =
            OppgaveClient("url", accessTokenClient, "scope", httpClientTest.httpClient)
        val kafkaRetryPublisher = mockkClass(KafkaRetryPublisher::class)

        beforeTest {
            every { kafkaRetryPublisher.publishOppgaveToRetryTopic(any(), any(), any()) } returns
                Unit
        ...
        context("test OpprettOppgave") {
            test("Opprett oppgave OK") {
                ...
                verify(exactly = 0) {
                    kafkaRetryPublisher.publishOppgaveToRetryTopic(any(), any(), any())
                }
            }
